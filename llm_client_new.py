...
class LLMClient:
    def __init__(self):
        ...
        # Инициализируем клиент, если библиотека доступна
        self.client = None
        if GROQ_AVAILABLE:
            try:
                # Создаем клиент Groq API
                self.client = groq.Client(api_key=api_key)
                # Проверяем доступность API с помощью легкого запроса
                models = self.client.models.list()
                if models and len(models.data) > 0:
                    print(f"Successfully connected to Groq API. Available models: {len(models.data)}")
                else:
                    print("WARNING: No models available from Groq API")
                    GROQ_AVAILABLE = False
